...
new p_enabled, p_show_jump, p_show_duck, p_show_fog, p_speed_mode, p_height

public plugin_init() {
    register_plugin("Frupre Consolidated", "4.7", "Custom")
    ...
    p_speed_mode = register_cvar("frupre_speed", "1") // 0=Off, 1=Horizontal, 2=3D
...
update_display(id, Float:fTime) {
    ...
    if (iMode > 0) {
        ...
        if (iMode == 2) // 3D (XYZ)
            iSpeed = floatround(floatsqroot(vel[0]*vel[0] + vel[1]*vel[1] + vel[2]*vel[2]))
        else // Horizontal (XY)
            iSpeed = floatround(floatsqroot(vel[0]*vel[0] + vel[1]*vel[1]))
            
        formatex(szSpeed, charsmax(szSpeed), "^n%d", iSpeed)
    }
