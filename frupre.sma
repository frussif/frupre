...
new g_pMode[33], g_pFog[33], g_pSpeed[33], g_pSType[33], g_pHeight[33], g_pColor[33], g_pHud[33], g_pEnabled[33], g_pScrollInfo[33], g_pShowStats[33]
...
public plugin_init() {
	register_plugin("Frupre Pro Ultimate", "11.5", "Stats-Standalone")
...
update_display(id, Float:fTime) {
    ...
    // Relevant if scroll used OR if we just landed a jump (g_finalDist > 0)
    new bool:isJumpRelevant = (showStats && (curFog <= 20 || g_finalDist[id] > 0.0))

    if (g_pHud[id] == 1) {
        static szMain[256], szStats[64], szFog[32], szSpeedLine[128], szPad[64]; szStats[0] = 0; szFog[0] = 0; szSpeedLine[0] = 0; szPad[0] = 0
        ...
        if (isJumpRelevant) {
            ...
            if (fogOn && curFog <= 20) formatex(szFog, 31, "%sFOG: %d", szStats[0] ? "^n" : "", curFog)
        }

        if (g_pSpeed[id] == 2 && showStats) {
            if (isJumpRelevant && g_finalGain[id] > 0) formatex(szSpeedLine, 127, "^n%d (%d)", g_staticSpeed[id], g_finalGain[id])
            else formatex(szSpeedLine, 127, "^n%d", g_staticSpeed[id])
            
            if (isJumpRelevant && g_pShowStats[id] && g_finalDist[id] > 0.0) {
                new szTmp[64]; formatex(szTmp, 63, "^n%d%%%% | %d/%d | %.1f", g_finalSync[id], g_finalOverlap[id], g_finalDead[id], g_finalDist[id])
                add(szSpeedLine, 127, szTmp)
            }
        }
        
        if (g_pSpeed[id] == 1) {
            new szLive[32]; formatex(szLive, 31, "^n%d", calculate_speed(id)); add(szSpeedLine, 127, szLive)
        }

        if (szStats[0] || szFog[0] || szSpeedLine[0]) { formatex(szMain, 255, "%s%s%s%s", szPad, szStats, szFog, szSpeedLine); client_print(id, print_center, szMain); }
    } else {
        new Float:startY = 0.35 + (float(g_pHeight[id]) * 0.025), Float:gap = 0.012
        static szLine1[64], szLine2[192]; szLine1[0] = 0; szLine2[0] = 0
        ...
        if (showStats) {
            ...
            if (isJumpRelevant) {
                r2 = 255; g2 = 255; b2 = 100 
                if (colorsOn && fogOn && curFog <= 20) { if (curFog <= 2) { r2 = 0; g2 = 255; b2 = 0; } else if (curFog == 3) { r2 = 255; g2 = 150; b2 = 0; } else { r2 = 255; g2 = 0; b2 = 0; } }
                if (fogOn && curFog <= 20) formatex(szLine2, 127, "FOG: %d", curFog)
            }

            if (g_pSpeed[id] == 2) {
                if (szLine2[0]) add(szLine2, 191, "^n")
                if (isJumpRelevant && g_finalGain[id] > 0) {
                    static szSp[32]; formatex(szSp, 31, "%d (%d)", g_staticSpeed[id], g_finalGain[id]); add(szLine2, 191, szSp)
                } else {
                    static szSp[32]; formatex(szSp, 31, "%d", g_staticSpeed[id]); add(szLine2, 191, szSp)
                }
            }

            if (isJumpRelevant && g_pShowStats[id] && g_finalDist[id] > 0.0) {
                if (szLine2[0]) add(szLine2, 191, "^n")
                static szSt[64]; formatex(szSt, 63, "%d%%%% | %d/%d | %.1f", g_finalSync[id], g_finalOverlap[id], g_finalDead[id], g_finalDist[id]); add(szLine2, 191, szSt)
            }
        }

        if (g_pSpeed[id] == 1) {
            if (szLine2[0]) add(szLine2, 191, "^n")
            static szLv[32]; formatex(szLv, 31, "%d", calculate_speed(id)); add(szLine2, 191, szLv)
        }

        if (szLine2[0]) { set_hudmessage(r2, g2, b2, -1.0, startY + gap, 0, 0.0, 0.1, 0.0, 0.0, 4); show_hudmessage(id, szLine2); }
    }
}
public menu_fru(id) {
...
public menu_handler(id, menu, item) {
	...
	switch(item) {
		...
		case 7: { save_data(id); client_print(id, print_chat, "[Frupre] Saved."); }
	}
...
public save_data(id) {
	...
	formatex(szLine, 127, "Color %d", g_pColor[id]); write_file(szLine, szLine)
	formatex(szLine, 127, "Hud %d", g_pHud[id]); write_file(szLine, szLine)
	formatex(szLine, 127, "Plugin %d", g_pEnabled[id]); write_file(szLine, szLine)
	formatex(szLine, 127, "ScrollInfo %d", g_pScrollInfo[id]); write_file(szLine, szLine)
	formatex(szLine, 127, "ShowStats %d", g_pShowStats[id]); write_file(szLine, szLine)
}
