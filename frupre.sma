...
public plugin_init() {
    register_plugin("Frupre Pro Ultimate", "11.9", "Stats-HeightFixed")
...
update_display(id, Float:fTime) {
    ...
    if (g_pHud[id] == 1) { // Center Print
        ...
        if (szBlock2[0]) {
            set_hudmessage(r2, g2, b2, -1.0, startY + 0.013, 0, 0.0, 0.1, 0.0, 0.0, 4)
...
public menu_fru(id) {
    ...
    // Allows the menu to close if 0 is pressed or another menu is called
