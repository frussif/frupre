...
public plugin_init() {
    register_plugin("Frupre Pro Ultimate", "27.8", "Full-Modular-DHUD")
...
public update_display(id, Float:fTime) {
    ...
    if (g_pScrollInfo[id] && curCount >= 2) {
        ...
        // The '0' before 0.05 is the channel. Setting it to 0 clears specific slots.
        // We use set_hudmessage style logic but for DHUD.
        set_dhudmessage(r, g, b, -1.0, startY, 0, 0.0, 0.1, 0.0, 0.0)
        show_dhudmessage(id, "%d [%d]", curCount, curStep)
    }

    // Main Stats - FIXED CHANNEL 1
    ...
    // Using a different Y offset for the second message ensures no overlap
